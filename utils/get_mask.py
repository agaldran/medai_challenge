...
def crop_to_fov(img, mask=None, coords=False):
    # create fov mask
    img = np.array(img)
    try:
        fov = get_only_mask(img)
    except:
        corrected = Image.fromarray(img_as_ubyte(exposure.adjust_sigmoid(img_as_float(img), cutoff=0.15)))
        fov = get_only_mask(np.array(corrected))

    fov = np.array(fov).astype(int)
    # get fov bbox coordinates
    minr, minc, maxr, maxc = regionprops(fov)[0].bbox
    # crop image and segmentation (if present)
    im_crop = np.array(img)[minr:maxr, minc:maxc]
    if mask is not None:
        mask_crop = np.array(mask)[minr:maxr, minc:maxc]
        if coords:
            return Image.fromarray(im_crop), Image.fromarray(mask_crop), (minr, minc, maxr, maxc)
        return Image.fromarray(im_crop), Image.fromarray(mask_crop)
    if coords:
        return Image.fromarray(im_crop), (minr, minc, maxr, maxc)
