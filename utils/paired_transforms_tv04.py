...
class RandomAffine(object):
    ...
    def __init__(self, degrees, translate=None, scale=None, shear=None,
                 resample=False, resample_tg=False, fillcolor=0, fillcolor_tg=0):
        if isinstance(degrees, numbers.Number):
            if degrees < 0:
                raise ValueError("If degrees is a single number, it must be positive.")
            self.degrees = (-degrees, degrees)
        ...
        self.fillcolor_tg = fillcolor_tg
    ...
    def __call__(self, img, target=None):
        ...
        if target is not None:
            return F.affine(img, *ret, resample=self.resample, fillcolor=self.fillcolor), \
                   F.affine(target, *ret, resample=self.resample_tg, fillcolor=self.fillcolor_tg)
                   # resample = False is by default nearest, appropriate for targets
        return F.affine(img, *ret, resample=self.resample, fillcolor=self.fillcolor)
